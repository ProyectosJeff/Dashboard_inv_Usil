...
// ======= Onboarding / Tour ADAPTATIVO =======
(function(){
  const SEEN_KEY = (() => {
    try{
      const u = (window.Common && window.Common.getCurrentUser && window.Common.getCurrentUser()) || {};
      return "tour_done_v2_" + (u.username || "anon");
    } catch { return "tour_done_v2_anon"; }
  })();
  ...
  const steps = [
    { title:"Mapea ESTATUS",
      text:"Ajusta el valor de Estatus y elige VALIDACION, y en el CAMPO ACTIVO elige COND.",
      target:"#map_estatus", place:"right" },
    { title:"Filtra por sede",
      text:"Usa el filtro de sede para ver solo una sede. Todo el tablero se sincroniza.",
      target:"#filter_sede", place:"right" },
    { title:"Rango de fechas",
      text:"Selecciona Desde/Hasta. Se incluyen ambos días completos.",
      target:"#filter_from", place:"bottom" },
    { title:"Indicadores",
      text:"Los cuadros muestran resultados tras aplicar todos los filtros.",
      target:"#kpiTotal", place:"top" }
   
  ];

  // ===== Utils de layout responsivo =====
  ...
  function clipToViewport(left, top, width, height){
    const vw = window.innerWidth, vh = window.innerHeight;
    const L = Math.max(16, Math.min(left, vw - width  - 16));
    const T = Math.max(16, Math.min(top,  vh - height - 16));
    return { left:L, top:T };
  }

  function placeCardNear(target, place){
    const rect = target.getBoundingClientRect();
    const card = el.card;
    const pad = 10;

    // Medimos tarjeta actual
    ...
    switch(place){
      case "right":  left += rect.width + pad; top += rect.height/2 - ch/2; break;
      case "left":   left -= (cw + pad);       top += rect.height/2 - ch/2; break;
      case "bottom": top  += rect.height + pad;left += rect.width/2 - cw/2; break;
      default:       top  -= (ch + pad);       left += rect.width/2 - cw/2; // top
    }

    const pos = clipToViewport(left, top, cw, ch);
    card.style.left = pos.left + "px";
    card.style.top  = pos.top  + "px";
  }

  function showArrow(target, place){
    const rect = target.getBoundingClientRect();
    const a = el.arrow;
    a.style.display = isMobile() ? "none" : "block";
    if (isMobile()) return;

    a.style.borderColor = "transparent";
    if (place === "right"){
      a.style.borderLeftColor = "#fff";
      a.style.left = (rect.right + window.scrollX) + "px";
      a.style.top  = (rect.top   + window.scrollY + rect.height/2 - 10) + "px";
    } else if (place === "left"){
      a.style.borderRightColor = "#fff";
      a.style.left = (rect.left + window.scrollX - 20) + "px";
      a.style.top  = (rect.top  + window.scrollY + rect.height/2 - 10) + "px";
    } else if (place === "bottom"){
      a.style.borderTopColor = "#fff";
      a.style.left = (rect.left + window.scrollX + rect.width/2 - 10) + "px";
      a.style.top  = (rect.bottom + window.scrollY - 20) + "px";
    } else {
      a.style.borderBottomColor = "#fff";
      a.style.left = (rect.left + window.scrollX + rect.width/2 - 10) + "px";
      a.style.top  = (rect.top  + window.scrollY - 0) + "px";
    }
  }

  function highlightTarget(target){
    const h = el.highlight;
    if (isMobile()){ h.style.display = "none"; return; }
    const r = target.getBoundingClientRect();
    h.style.display = "block";
    h.style.top    = (r.top  + window.scrollY - 6) + "px";
    h.style.left   = (r.left + window.scrollX - 6) + "px";
    h.style.width  = (r.width  + 12) + "px";
    h.style.height = (r.height + 12) + "px";
  }

  function placeCardMobile(){
    // “Bottom sheet”
    const c = el.card;
  ...
  function showStep(i){
    ...
    el.next.textContent  = (i === steps.length - 1) ? "Terminar" : "Siguiente";

    // Colocar según viewport
    requestAnimationFrame(()=>{
      if (isMobile()){
        placeCardMobile();
        highlightTarget(target); // se ocultará en CSS móvil
        el.arrow.style.display = "none";
      } else {
        placeCardNear(target, s.place || "top");
        highlightTarget(target);
        showArrow(target, s.place || "top");
      }
    });

    // Reposicionar en resize/scroll (debounce simple)
    const onR = () => { showStep(i); };
    const onS = () => { /* throttle ligero */ clearTimeout(onS._t); onS._t = setTimeout(()=>showStep(i), 80); };
    window.addEventListener("resize", onR);
    window.addEventListener("scroll", onS, { passive:true });

    // Observer por si cambia tamaño del target
    const ro = new ResizeObserver(()=> showStep(i));
    try { ro.observe(target); } catch {}

    // Guardar para limpiar al avanzar
    cleanups.push(()=> {
  ...
  el.skip.addEventListener("click", ()=>{ localStorage.setItem(SEEN_KEY,"1"); hideTour(); });
  el.backdrop.addEventListener("click", ()=>{ localStorage.setItem(SEEN_KEY,"1"); hideTour(); });
